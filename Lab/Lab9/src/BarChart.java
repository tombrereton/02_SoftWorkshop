...
public class BarChart extends JPanel {
    ...
    public static int xOffset() {
        return barWidth() + barGap(); // This needs to be filled
    }
    ...
    public static int getMaximal(ArrayList<Measure> measures) {
        if (measures.size() == 0) {
            ...
            for (int i = 0; i < measures.size(); i++) {
                if (measures.get(i).getValue() > maximum) maximum = measures.get(i).getValue();
            }
            return maximum;
        }
    }
    ...
    public void paintComponent(Graphics g) {
        ...
        if (numberOfMeasures <= 200) {
            if (numberOfMeasures <= 20) {
                ...
                barGap = 200 / numberOfMeasures; // We have full size of 10 pixels for short ArrayLists
                // We adjust the size for ArrayLists between 21 and 200
                // Add further cases here.
                for (int i = 0; i < numberOfMeasures; i++) {
                    normalizedY = normalize(measures.get(i).getValue(), max);

                    g.fillRect(barGap() + i * xOffset(),
                            yNumberOfPixels - normalizedY,
                            barWidth(),
                            normalizedY);
                    if (numberOfMeasures <= 30) {
                        g.drawString(this.measures.get(i).getDescription(), barGap() + i * xOffset(), yNumberOfPixels + 20);
                    }
                }
            }
        } else if (numberOfMeasures <= 600) {
            int[] xValues = new int[numberOfMeasures + 2];
            int[] yValues = new int[numberOfMeasures + 2];

            for( int i = 0; i <numberOfMeasures; i++){
                xValues[i +1] = i;
                yValues[i + 1] = yNumberOfPixels - normalize(measures.get(i).getValue(), max);
            }
            xValues[0] = 0;
            yValues[0] = yNumberOfPixels;
            xValues[numberOfMeasures + 1] = numberOfMeasures+1;
            yValues[numberOfMeasures + 1] = yNumberOfPixels;
            g.fillPolygon(xValues, yValues, numberOfMeasures + 2);
    ...
    public static String randomName() {
        int offset = (int) 'A';
        int r1 = (int) (26 * Math.random()) + offset;
        int r2 = (int) (26 * Math.random()) + offset;
    ...
    public static void main(String[] args) {
        try {
            ...
            System.out.println(getMaximal(measures));
            System.out.println(normalize(100, 200));
